...
**Trading research.** Public-data ingest and backtesting for spot crypto on Kraken, with an equities data companion. Cost-aware walk-forward testing against real tiered fees, deterministic signals only, no live execution.

**Detour.** A phone-first travel planner built on the Claude API. Multi-day itineraries and location-aware local answers, with a place graph that makes each answer cheaper than the last. Itinerary generation runs as a background job, credits and affiliates pay for the tokens, and it doubles as our public generative-AI portfolio piece.
...
| `BDS_bpo` | BPO Autopilot hosted web application, multi-tenant with row-level security | Production |
| `bpo_autofill` | The original BPO fill tool. Python desktop prototype that ran the first live orders and proved the extraction and fill approach | Superseded by `BDS_bpo` |
| `BDS_website` | Marketing site (Next.js) | Live |
| `BDS_assets` | Beyond Assets financial tracker | Production |
| `BDS_VaReferrals` | HIPAA-aware referral tracker for a medical practice, built on Microsoft 365 | Client delivery |
| `BDS_Trade` | Kraken spot crypto ingest and backtesting platform | Research |
| `bds_equities` | Equities market data ingest, companion to `BDS_Trade` | Research |
| `BDS_Travel` | Detour, a phone-first AI travel planner on the Claude API. Our public generative-AI portfolio piece | In build |
| `BDS_vault` | Obsidian knowledge base: doctrine, proof registry, night-run specs | Active |
| `BDS` | Organization home | |
